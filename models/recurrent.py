...
class Seq2Seq(nn.Module):
	...
	def __init__(self,
		...
		if not self.use_bpe:
			if self.load_embedding:
				...
				self.embedder = nn.Embedding(
					self.vocab_size, self.embedding_size, sparse=False, padding_idx=PAD)
		else:
			# BPE
			embedding_matrix = np.random.rand(self.vocab_size, self.embedding_size)
			...
			self.embedder = nn.Embedding.from_pretrained(
				embedding_matrix, freeze=False, sparse=False, padding_idx=PAD)
		...
		if not self.residual:
			...
			lstm_uni_dec_first = torch.nn.LSTM(self.embedding_size + self.hidden_size_shared,
									self.hidden_size_dec,
		...
		self.out = nn.Linear(self.hidden_size_shared , self.vocab_size, bias=True)
	...
	def check_var(self, var_name, var_val_set=None):
		...
		if not hasattr(self, var_name):
			if var_name == 'additional_key_size':
				...
				var_val = var_val_set if type(var_val_set) != type(None) else 'null'
			else:
	...
	def beam_search_decoding(self, att_keys, att_vals,dec_hidden=None,
		...
		input_var = Variable(torch.transpose(torch.LongTensor(
			[[BOS] * self.batch_size * self.beam_width]), 0, 1)).to(device=device)
	...
	def _backtrack(self, nw_output, nw_hidden, predecessors, symbols, scores, b, hidden_size):
		...
		if lstm:
			...
			h_n = tuple([torch.zeros(state_size)
				.to(device=device), torch.zeros(state_size).to(device=device)])
		else:
